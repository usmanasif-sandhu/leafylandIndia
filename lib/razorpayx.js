...
function bankDetailsFingerprint(details) {
    return createHash('sha256')
        .update(`${details.accountName}|${details.ifscCode}|${details.bankAccount}`)
        .digest('hex')
}
 ...
 * Cache is fingerprint-bound: editing bank details invalidates it.
 */
export async function ensureFundAccount(store) {
    ...
    const fingerprint = bankDetailsFingerprint(details)
    if (
        settings.razorpayFundAccountId &&
        settings.razorpayContactId &&
        settings.razorpayFundAccountFp === fingerprint
    ) {
    ...
    await prisma.store.update({
        ...
        data: {
            settings: {
                ...
                razorpayFundAccountFp: fingerprint,
            },
        },
    })
