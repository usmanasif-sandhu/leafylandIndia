 ...
 *
 * CRASH-GAP NOTE: because the gateway payout is created BEFORE earnings are claimed, a process crash between createXPayout and the claiming transaction leaves an orphaned gateway payout (no Payout row). Recovery: locate the transfer in the RazorpayX dashboard by reference_id store_<storeId> and reconcile manually; webhook events for payout ids unknown to the DB are intentionally acked and ignored.
 */
export async function releaseStorePayout({ storeId, commissionPaiseOverride = null, reference = null }) {
    ...
    try {
        ...
        if (xPayout?.id && !(await cancelXPayout(xPayout.id))) {
            // Gateway payout already left `queued` and could not be cancelled — money may move
            // with no Payout row. Webhooks for this id will be acked silently (unknown id),
            // so manual reconciliation in the RazorpayX dashboard is required.
            console.error(
                `[PAYOUT-ORPHAN] Gateway payout ${xPayout.id} for store ${storeId} (${netPaise} paise) could not be cancelled after a failed release. Reconcile manually via reference_id store_${storeId}.`,
            )
        }
        throw e
    }
