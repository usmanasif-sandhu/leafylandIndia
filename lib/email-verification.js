...
export async function sendVerificationEmail(email, name) {
    ...
    if (!isEmailConfigured()) {
        // No email transport is available. Expose the verification link so the
        // app stays usable (dev, demo, or a server without SMTP configured).
        // Real deployments should configure SMTP so the link is emailed instead.
        console.warn('[email] Email transport not configured. Verification link:', verifyUrl)
