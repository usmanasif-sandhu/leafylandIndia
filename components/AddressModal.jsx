...
const AddressModal = ({ setShowAddressModal }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // TODO: replace with your actual endpoint once the address API route exists
    ...
    return (
        <form onSubmit={e => toast.promise(handleSubmit(e), { loading: 'Adding Address...', success: 'Address saved', error: 'Could not save address' })} className="fixed inset-0 z-50 bg-white/60 backdrop-blur h-screen flex items-center justify-center">
            <div className="flex flex-col gap-5 text-slate-700 w-full max-w-sm mx-6">
                ...
                <div className="flex gap-4">
                    <input name="zip" onChange={handleAddressChange} value={address.zip} className="p-2 px-4 outline-none border border-slate-200 rounded w-full" type="text" inputMode="numeric" placeholder="Zip code" required />
                ...
                <button className="bg-emerald-900 text-white text-sm font-medium py-2.5 rounded-md hover:bg-emerald-950 active:scale-95 transition-all">SAVE ADDRESS</button>
            </div>
